"""Ingest metadata dataset palm (Valles-Coral et al. 2025, UNSM Tarapoto, Peru)
dan terjemahkan skema berbahasa Spanyol menjadi Inggris.

Dataset asli hanya berisi video palm (kepalan tangan yang dibuka bertahap)
dan metadata.csv dengan nama kolom dan label kategori berbahasa Spanyol.
Modul ini menstandardisasi skema tersebut ke Inggris agar konsisten dengan
kode situs lain di proyek ini, tanpa mengubah isi data itu sendiri.
"""
...
SITE_NAME = "palm"
DATASET_NAME = "palm_valles_coral"
...
def raw_dir() -> Path:
    """Path folder raw dataset palm (hasil download langsung dari Google Drive)."""
    return dataset_root(SITE_NAME) / "raw"


def translate_metadata(df: pd.DataFrame) -> pd.DataFrame:
    """Terjemahkan nama kolom metadata palm dari Spanyol menjadi Inggris.

    Nilai kategorikal biner (0/1) dan nilai Sex (M/F) sudah netral bahasa
    sehingga tidak perlu diterjemahkan, hanya nama kolomnya.
    """
...
def _resolve_video_path(root: Path, category: str, identifier: str, filename: str) -> Path | None:
    """Cari path video palm untuk satu baris metadata di folder kategori terkait.

    File di disk dinamai memakai ID partisipan (mis. ID004.mp4), bukan nilai
    kolom PALM_FILE yang menyimpan nama file timestamp asli dari akuisisi data
    dan tidak konsisten dengan nama file hasil unduhan. Kolom PALM_FILE dipakai
    sebagai fallback lewat pencocokan stem bila pencarian berbasis ID gagal.
    """
    folder = root / "palmas" / category
    by_id = folder / f"{identifier}.mp4"
    if by_id.exists():
        return by_id
    ...
    matches = sorted(folder.glob(f"{stem}.*")) if folder.is_dir() else []
    return matches[0] if matches else None


def build_manifest(save: bool = False, output_dir: Path | None = None) -> pd.DataFrame:
    """Bangun manifest dataset palm dari metadata_en.csv dan video di folder palmas.

    Severity diturunkan dari kolom one-hot Normal/Mild/Moderate dan dipetakan ke
    kosakata severity bersama proyek (SEVERITY_ORDER pada src.common.models),
    sedangkan label anemik diturunkan dari ambang hemoglobin dewasa sesuai
    gender, konsisten dengan pendekatan situs konjungtiva. Baris tanpa kategori
    keparahan yang jelas atau tanpa file video yang berhasil diunduh dilewati.
    """
    root = raw_dir()
    ...
    for _, record in meta.iterrows():
        ...
        if category is None:
            ...
            continue
        video_path = _resolve_video_path(root, category, identifier, str(record.get("PALM_FILE", "")))
    ...
    if missing:
        preview = missing[:8]
        print(f"Palm melewati {len(missing)} baris (video tidak valid atau Hb/Age tidak plausibel): {preview}")
...
def summarize(frame: pd.DataFrame) -> None:
    """Cetak ringkasan eksplorasi distribusi utama pada manifest palm."""
